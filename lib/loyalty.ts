...
export const TIER_META: Record<LoyaltyTier, { label: string; color: string; bg: string; emoji: string }> = {
  platinum: { label: "Platinum", color: "#1e293b", bg: "#e2e8f0", emoji: "💎" },
  gold:     { label: "Gold",     color: "#92400e", bg: "#fef3c7", emoji: "🥇" },
  silver:   { label: "Silver",   color: "#64748b", bg: "#f1f5f9", emoji: "🥈" },
  bronze:   { label: "Bronze",   color: "#78350f", bg: "#fef9c3", emoji: "🥉" },
  none:     { label: "Member",   color: "#ea580c", bg: "#ffedd5", emoji: "⭐" },
};
...
export function awardPoints(
  ...
  appendHistory({ clientId: client.id, type: "earn", points: pts, note: `Earned for Rs. ${amount} appointment`, appointmentId });
  return updated;
}
