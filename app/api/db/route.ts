...
const ALLOWED = new Set(["clients", "appointments", "staff", "services", "inventory", "invoices", "expenses", "attendance", "payouts", "cash_flow_income", DELETED_RECORDS_ENTITY]);
...
async function loadDeletedIds(userId: string, locationId: string, entity: string): Promise<Set<string>> {
  try {
    const result = await db.execute({
      sql: "SELECT data FROM business_data WHERE entity = ?",
 ...
 * caller's own business data (userId/locationId are resolved from the session,
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    await ensureTable();
    const key = storageKey(userId, locationId, entity);
    await backupExistingBusinessData(key, userId);
    ...
    if (entity === DELETED_RECORDS_ENTITY) {
      ...
      const existing = await db.execute({ sql: "SELECT data FROM business_data WHERE entity = ?", args: [key] });
 ...
 *
 * Permanently deletes every business_data row that belongs to one branch (all
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    if (locationId === "main") {
      ...
      const result = await db.execute({
        sql: "SELECT entity FROM business_data WHERE entity LIKE ?",
    ...
    for (const key of keys) {
      ...
      await backupExistingBusinessData(key, userId, "before-write");
      await db.execute({ sql: "DELETE FROM business_data WHERE entity = ?", args: [key] });
    }
