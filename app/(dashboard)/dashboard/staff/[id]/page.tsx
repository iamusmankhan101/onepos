...
const STATUS_META: Record<string, { label: string; color: string; bg: string }> = {
  ...
  booked:       { label: "Booked",      color: "#EA580C", bg: "#FFEDD5" },
                ...
                : servicesList.map((sv) => {
                  ...
                  return (
                    <div key={sv.id} onClick={() => toggleService(sv.id)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "6px 8px", borderRadius: 6, cursor: "pointer", background: checked ? "#f4f0fe" : "transparent" }}>
                      <div style={{ width: 16, height: 16, borderRadius: 4, border: "1px solid #EA580C", display: "flex", alignItems: "center", justifyContent: "center", background: checked ? "#EA580C" : "#fff", flexShrink: 0 }}>
  ...
  if (notFound) return (
    <div className="dash-page" style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "60vh" }}>
      <div style={{ textAlign: "center" }}>
        ...
        <button onClick={() => router.push("/dashboard/staff")} style={{ marginTop: 8, padding: "10px 24px", borderRadius: 10, background: "#EA580C", border: "none", color: "#fff", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
          Back to Staff
        </button>
      </div>
    </div>
      ...
      <div style={{ background: `linear-gradient(135deg, ${staff.color}22 0%, ${staff.color}08 100%)`, borderBottom: "1px solid #ebebf0" }}>
        <div className="dash-page" style={{ paddingTop: 20, paddingBottom: 28 }}>
          ...
          <button
            ...
            style={{ display: "flex", alignItems: "center", gap: 6, background: "none", border: "none", cursor: "pointer", color: "#EA580C", fontSize: 13, fontWeight: 700, marginBottom: 20, padding: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 16 }}>
            ...
            <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
              <button
                onClick={() => exportStaffPdf(staff, appointments, settingsStore.business.name as string)}
              ...
              <button
                ...
                style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 20px", borderRadius: 10, border: "1px solid #EA580C", background: "#fff", color: "#EA580C", fontSize: 13, fontWeight: 700, cursor: "pointer", boxShadow: "0 1px 4px rgba(234,88,12,0.12)" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 22 }}>
          ...
          <Section title="Revenue Breakdown" icon={BarChart2}>
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <RevRow label="This Week"   value={stats.revWeek}      pct={Math.round((stats.revWeek / maxRev) * 100)}      color="#EA580C" />
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {stats.topServices.map((svc) => (
                  <div key={svc.name} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <MiniBar value={svc.count} max={maxSvc} color="#EA580C" />
                    <div style={{ width: 28, textAlign: "right", fontSize: 12, fontWeight: 700, color: "#EA580C", flexShrink: 0 }}>{svc.count}×</div>
                  </div>
                ))}
              </div>
            )}
                  ...
                  {recentCompleted.map((appt, i) => {
                    ...
                    return (
                      <tr key={appt.id}
                        ...
                        <td style={{ padding: "10px 12px", fontSize: 13, fontWeight: 700, color: "#EA580C", whiteSpace: "nowrap" }}>{fmt(appt.totalAmount ?? 0)}</td>
                      </tr>
                    );
                  })}
                </tbody>
                ...
                {stats.upcoming.slice(0, 6).map((appt) => {
                  ...
                  return (
                    <div key={appt.id} style={{ display: "flex", flexDirection: "column", gap: 4, padding: "10px 12px", borderRadius: 10, background: "#f9f9fb", border: "1px solid #f0f0f8" }}>
                      ...
                      <div style={{ fontSize: 11, color: "#EA580C", fontWeight: 600 }}>{appt.serviceNames[0]}</div>
                    </div>
                  );
                })}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {stats.topClients.map((cl, i) => (
                  <div key={cl.name + i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 10px", borderRadius: 9, background: "#f9f9fb" }}>
                    ...
                    <div style={{ fontSize: 12, fontWeight: 700, color: "#EA580C", flexShrink: 0 }}>{fmt(cl.spent)}</div>
                  </div>
                ))}
              </div>
            )}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {assignedServices.map((sv) => (
                  <div key={sv.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 12px", borderRadius: 9, background: "#f9f9fb", border: "1px solid #f0f0f8" }}>
                    ...
                    <span style={{ fontSize: 12, color: "#EA580C", fontWeight: 700 }}>{fmt(sv.price)}</span>
                  </div>
                ))}
              </div>
            )}
