...
const STATUS_CONFIG = {
  ...
  arrived:       { color: "#F97316", bg: "#fff7ed" },
  "in-progress": { color: "#d97706", bg: "#fffbeb" },
};

const TAG_COLORS: Record<string, { color: string; bg: string }> = {
  VIP:      { color: "#EA580C", bg: "#FFEDD5" },
  Regular:  { color: "#059669", bg: "#ecfdf5" },
  Wholesale:{ color: "#db2777", bg: "#fdf2f8" },
...
function ClientPanel({ client, onClose, appointments, locations, onUpdate, onDelete }: { client: Client; onClose: () => void; appointments: Appointment[]; locations: BusinessLocation[]; onUpdate?: (c: Client) => void; onDelete?: (id: string) => void }) {
  ...
  type HistoryEntry = { kind: "appt"; data: Appointment } | { kind: "pos"; data: Invoice };
  ...
  return (
    <div onClick={onClose} className="modal-overlay" style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      <div onClick={(e) => e.stopPropagation()} className="modal-sheet" style={{ background: "#fff", borderRadius: 20, width: 500, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
        ...
        {/* Header */}
        <div style={{ background: "linear-gradient(135deg, #FFEDD5, #fdf2f8)", padding: "28px 24px 20px", borderBottom: "1px solid #f0f0f8", position: "relative" }}>
          <div style={{ position: "absolute", top: 16, right: 16, display: "flex", gap: 8 }}>
            <button
              onClick={() => { onClose(); panelRouter.push(`/dashboard/pos?client=${encodeURIComponent(client.id)}`); }}
              title="Open the POS with this customer selected"
              style={{ display: "flex", alignItems: "center", gap: 5, padding: "5px 12px", borderRadius: 7, border: "none", background: "var(--accent-gradient)", fontSize: 11, fontWeight: 700, color: "#fff", cursor: "pointer" }}
            >
              <ShoppingCart size={11} /> New Sale
            </button>
            <button
              onClick={() => { onClose(); panelRouter.push(`/dashboard/clients/${client.id}`); }}
              style={{ display: "flex", alignItems: "center", gap: 5, padding: "5px 12px", borderRadius: 7, border: "1px solid #FFEDD5", background: "#fff", fontSize: 11, fontWeight: 600, color: "#EA580C", cursor: "pointer" }}
            >
              <ExternalLink size={11} /> Full Profile
            </button>
            {!editing && (
              <button onClick={() => setEditing(true)} style={{ padding: "5px 12px", borderRadius: 7, border: "1px solid #FFEDD5", background: "#fff", fontSize: 11, fontWeight: 600, color: "#EA580C", cursor: "pointer" }}>
                Edit
              </button>
            )}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            <div style={{ width: 64, height: 64, borderRadius: "50%", background: "linear-gradient(135deg, #9A3412, #F97316)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 24, fontWeight: 700, color: "#fff" }}>
                  ...
                  return (
                    <div key={a.id}>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "9px 12px", borderRadius: isExpanded ? "10px 10px 0 0" : 10, background: "#f9f9fb", border: "1px solid #f0f0f8", borderBottom: isExpanded ? "none" : "1px solid #f0f0f8" }}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <div style={{ fontSize: 13, fontWeight: 800, color: "#EA580C" }}>{fmt(a.totalAmount)}</div>
                          <button
                            onClick={() => setExpandedVisit(isExpanded ? null : a.id)}
                            style={{ display: "flex", alignItems: "center", gap: 4, padding: "4px 9px", borderRadius: 20, border: "none", cursor: "pointer", background: photoCount > 0 ? "#ffedd5" : "#f3f4f6" }}
                          >
                            <Camera size={11} color={photoCount > 0 ? "#EA580C" : "#9ca3af"} />
                            <span style={{ fontSize: 10, fontWeight: 700, color: photoCount > 0 ? "#EA580C" : "#9ca3af" }}>
                      ...
                      {isExpanded && (
                        <div style={{ background: "#fff7ed", border: "1px solid #ffedd5", borderTop: "none", borderRadius: "0 0 10px 10px", padding: "12px" }}>
                          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                            {(["before", "after"] as const).map((side) => (
                              <div key={side}>
                                <div style={{ fontSize: 10, fontWeight: 700, color: "#ea580c", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 6 }}>{side}</div>
                                {photos[side] ? (
                                  ...
                                  <label style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 5, height: 90, borderRadius: 8, border: "2px dashed #fb923c", background: "#fff7ed", cursor: "pointer" }}>
                                    <Camera size={16} color="#fb923c" />
                                    <span style={{ fontSize: 10, fontWeight: 600, color: "#fb923c" }}>Upload {side}</span>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                ...
                <div style={{ display: "flex", gap: 8, paddingTop: 4 }}>
                  ...
                  <button onClick={() => {
                    ...
                    style={{ flex: 2, padding: "10px 0", borderRadius: 9, border: "none", background: "#EA580C", fontSize: 13, fontWeight: 600, color: "#fff", cursor: "pointer" }}>
                    Save Changes
                  </button>
                </div>
              </div>
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.55)",zIndex:400,display:"flex",alignItems:"center",justifyContent:"center",padding:16 }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:"#fff",borderRadius:18,width:"100%",maxWidth:520,padding:28,boxShadow:"0 20px 60px rgba(0,0,0,0.2)" }}>
        ...
        {step === "pick" && (
          <>
            <label style={{ display:"block",border:"2px dashed #fed7aa",borderRadius:14,padding:"32px 20px",textAlign:"center",cursor:"pointer",background:"#faf9ff",transition:"border-color 0.2s" }}
            ...
            <div style={{ marginTop:18,padding:"14px 16px",background:"#fff7ed",borderRadius:12,fontSize:12,color:"#9A3412",lineHeight:1.8 }}>
                ...
                <button
                  onClick={async () => {
                    ...
                    const sample = [
                      ...
                      { "Name":"Hina Malik",  "Phone":"923011112233","Email":"",                "Gender":"female","Date of Birth":"",          "Location":"Main Branch","Source":"manual",  "Tags":"Wholesale",    "Notes":"Bulk buyer",     "Total Visits":1,"Total Spend":8000, "Loyalty Points":200,"Created At":"2024-06-01" },
                    ];
                  ...
                  style={{ display:"flex",alignItems:"center",gap:5,padding:"5px 12px",borderRadius:8,border:"1px solid #fdba74",background:"#fff",fontSize:11,fontWeight:700,color:"#9A3412",cursor:"pointer",whiteSpace:"nowrap" }}>
              ...
              <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:"2px 16px",fontSize:11 }}>
                {[
                  ...
                  <div key={col}>
                    <span style={{ fontWeight:700,color:"#7c2d12" }}>{col}</span>
                    <span style={{ color:"#ea580c",marginLeft:4 }}>— {hint}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <>
              ...
              <div style={{ maxHeight:240,overflowY:"auto",border:"1px solid #e8e8f0",borderRadius:12,marginBottom:16 }}>
                <table style={{ width:"100%",borderCollapse:"collapse",fontSize:12 }}>
                  <thead><tr style={{ background:"#fff7ed" }}>
                    {["Name","Phone","Email","Tags"].map(h=><th key={h} style={{ padding:"8px 12px",textAlign:"left",fontWeight:700,color:"#9A3412",fontSize:11 }}>{h}</th>)}
                  </tr></thead>
                  <tbody>
            ...
            <div style={{ display:"flex",gap:10 }}>
              ...
              <button onClick={confirmImport} disabled={parsed.length===0}
                style={{ flex:2,padding:"11px 0",borderRadius:10,border:"none",background:parsed.length>0?"linear-gradient(135deg,#9A3412,#F97316)":"#e8e8f0",fontSize:13,fontWeight:700,color:parsed.length>0?"#fff":"#b0b0c8",cursor:parsed.length>0?"pointer":"not-allowed" }}>
        ...
        {step === "done" && result && (
          <div style={{ textAlign:"center",padding:"12px 0" }}>
            ...
            <button onClick={onClose} style={{ padding:"11px 36px",borderRadius:10,border:"none",background:"linear-gradient(135deg,#9A3412,#F97316)",fontSize:13,fontWeight:700,color:"#fff",cursor:"pointer" }}>Done</button>
          </div>
        )}
      </div>
    </div>
...
export default function ClientsPage() {
  ...
  const [locations, setLocations] = useState<BusinessLocation[]>(() => getBusinessLocations());
  ...
  useEffect(() => {
    function refreshLocations() {
      setLocations(getBusinessLocations());
    ...
    <div className="dash-page dashboard-polish" style={{ background: "#ffffff", minHeight: "100vh", display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      {showAdd && (
        <AddClientModal
          ...
          locations={locations}
          allowLocationSelection={locations.length > 1}
          clients={clients}
          onAdd={(newC) => {
            setClients((prevClients) => {
      ...
      <div className="dashboard-topbar page-header" style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <button
            ...
            style={{ display: "flex", alignItems: "center", gap: 7, padding: "10px 16px", borderRadius: 12, border: "1px solid rgba(234, 88, 12, 0.15)", background: "rgba(234, 88, 12, 0.04)", fontSize: 13, fontWeight: 750, color: "var(--accent)", cursor: "pointer", transition: "all 0.15s" }}
            className="hover-bg-light"
          >
          ...
          <div style={{ position: "relative" }}>
            <button
              ...
              style={{ display: "flex", alignItems: "center", gap: 7, padding: "10px 16px", borderRadius: 12, border: "1px solid rgba(234, 88, 12, 0.15)", background: "rgba(234, 88, 12, 0.04)", fontSize: 13, fontWeight: 750, color: clients.length === 0 ? "#b0b0c8" : "var(--accent)", cursor: clients.length === 0 ? "not-allowed" : "pointer", transition: "all 0.15s" }}
              className="hover-bg-light"
            >
      ...
      <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
        ...
        <button onClick={() => setShowFilters(!showFilters)} style={{ display: "flex", alignItems: "center", gap: 7, padding: "10px 18px", borderRadius: 12, border: `1px solid ${activeFilters > 0 ? "var(--accent-light)" : "#e3e0eb"}`, background: activeFilters > 0 ? "rgba(234, 88, 12, 0.04)" : "#fff", fontSize: 13, fontWeight: 750, color: activeFilters > 0 ? "var(--accent)" : "#6b6b8a", cursor: "pointer", transition: "all 0.15s" }} className="hover-bg-light">
          Filter
      ...
      {showFilters && (
        <div style={{
          ...
          flexWrap: "wrap",
          alignItems: "flex-end",
          boxShadow: "0 8px 24px rgba(75,40,20,.03)"
        }}>
          {[
      ...
      <div className="table-scroll-wrap" style={{
        ...
        borderRadius: 18,
        border: "1px solid rgba(226,223,235,.95)",
        boxShadow: "0 8px 28px rgba(75,40,20,.04)",
        overflow: "hidden"
      }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "40px 1.2fr 1.1fr 1fr 1fr 1.1fr 90px 110px 130px", padding: "12px 20px", borderBottom: "1px solid #f0f0f5", background: "#faf9fd", alignItems: "center" }}>
          <div style={{ display: "flex", alignItems: "center" }}>
            <input
              ...
              style={{ width: 15, height: 15, cursor: "pointer", accentColor: "#EA580C" }}
            />
          </div>
        ...
        {filtered.length === 0 ? (
          ...
          return (
            <div
              ...
              style={{ display: "grid", gridTemplateColumns: "40px 1.2fr 1.1fr 1fr 1fr 1.1fr 90px 110px 130px", padding: "14px 20px", borderBottom: isLast ? "none" : "1px solid #f8f8fc", alignItems: "center", cursor: "pointer", background: isChecked ? "#FFF7ED" : "transparent", transition: "background 0.2s" }}
            ...
            >
              ...
              {/* Client */}
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <div style={{ width: 34, height: 34, borderRadius: "50%", background: "linear-gradient(135deg, #F9731615, #ec489915)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: "#F97316", flexShrink: 0, border: "1.5px solid rgba(255,255,255,0.8)", boxShadow: "0 2px 4px rgba(0,0,0,0.04)" }}>
                ...
                <div>
                  ...
                  <div style={{ display: "flex", gap: 4, marginTop: 3, flexWrap: "wrap" }}>
                    {client.tags.map((tag) => {
                      const tc = TAG_COLORS[tag] ?? { color: "#6b6b8a", bg: "#FFF7ED" };
        ...
        <div className="mobile-only">
          {filtered.length === 0 ? (
            ...
            return (
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ width: 34, height: 34, borderRadius: "50%", background: "linear-gradient(135deg, #F9731615, #ec489915)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: "#F97316", flexShrink: 0 }}>
                    {client.name.charAt(0).toUpperCase()}
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
